...
class Store:
    ...
    def receive(self, delivery, digest, event, payload, source='github_webhook'):
        ...
        with self.db() as db:
            ...
            if action == 'closed':
                db.execute("UPDATE reviews SET state=CASE WHEN state IN ('queued','draft') THEN 'closed' ELSE state END, body=body || %s,event_at=%s,updated_at=now() WHERE repo_id=%s AND pr=%s", (Jsonb({'stale': True, 'pr_state': 'MERGED' if pr.get('merged') else 'CLOSED'}), when, repo_id, number))
                return {'accepted': True}
    ...
    def runs(self, run_id=None):
        ...
        return [{**r['body'], 'status': r['state'], 'queue_position': queued.index(r['id']) + 1 if r['id'] in queued else 0,
                 'deliveries': [{**d, **d['body'], 'ok': d['state']=='delivered'} for d in deliveries if d['run_id']==r['id']]} for r in rows]
    ...
    def recover(self, run_id, evidence):
        ...
        with self.db() as db:
            ...
            row = db.execute("UPDATE reviews SET state='blocked',lease_token=NULL,body=body || %s WHERE id=%s AND state='interrupted' RETURNING id", (Jsonb(patch),run_id)).fetchone()
            if not row:
    ...
    def claim(self, worker, repositories=None):
        with self.db() as db:
            ...
            if db.execute("SELECT id FROM reviews WHERE state IN ('running','interrupted') LIMIT 1").fetchone():
                return None
            row = db.execute("SELECT * FROM reviews WHERE state='queued' AND (%s::text[] IS NULL OR lower(body->>'repo')=ANY(%s::text[])) ORDER BY created_at,id FOR UPDATE SKIP LOCKED LIMIT 1", (repositories, repositories)).fetchone()
            if not row:
                return None
